...
#         print(users)
#         return render_template('user_results.html', users=users, colors= colors)
#     else:
...
def search_users():
    ...
    if not q:
        return render_template('search.html',users={"users": True})
    params = {
        "q": q,
        "sort": "followers",
        "order": "desc",
...
def trending_repos():
    ...
    if response.status_code == 200:
        ...
        for item in items:
            ...
            repos.append(temp)
        return render_template('repo_results.html', repos=repos, colors=colors)

    else:
        return f"Error: status code - {response.status_code}"

# @app.route('/repos/<key_word>', methods=['GET'])
# def search_repos(key_word):
...
def search_repos():
    ...
    q = request.args.get("q")
    if not q:
        return render_template('search.html',repos={"repos": True})
